...
#            soup = BeautifulSoup(data,'lxml')
        ...
        #针对性更改,可设置多个result
        try:
#            title = soup.select('.name').find('td')
            result = soup.select('.jsxx')[0].find_all('tr')
...
#        soup = BeautifulSoup(res.text,'lxml')   
            ...
            data = drive.page_source
            soup = BeautifulSoup(data,'lxml')
            result = soup.select('#padding')[0].find_all('a')
...
urls =[] 
u1 = 'http://kjxy.dufe.edu.cn/teacher/teacher.php?action=list&dstype=55&page='
for i in range(6):
    i += 1
    urls.append(u1 + str(i))
urls = get_urls(urls,'http://kjxy.dufe.edu.cn/') 
urls = list(set(urls))
#for item in urls:
#    print(item)
