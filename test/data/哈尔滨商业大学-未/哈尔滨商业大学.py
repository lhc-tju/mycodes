...
#            soup = BeautifulSoup(data,'lxml')
        ...
        try:
            title = soup.find('h2')
            result = soup.select('.v_news_content')[0].find_all('p')
#            r2 = soup.select('.tab-content')[0].find_all('div')
#            r2 = soup.find('tbody').find_all('p')
        except:
            try:
                result = soup.select('.v_news_content')[0].find_all('tr')
            except:
...
#        soup = BeautifulSoup(res.text,'lxml')   
            ...
            data = drive.page_source
            soup = BeautifulSoup(data,'lxml')
            result = soup.select('.main_conRCb')[0].find_all('a')
...
urls = ['http://czxy.hrbcu.edu.cn/dwjs/jsjj/1.htm','http://czxy.hrbcu.edu.cn/dwjs/jsjj/2.htm','http://czxy.hrbcu.edu.cn/dwjs/jsjj.htm']
urls = get_urls(urls,'http://czxy.hrbcu.edu.cn/')
urls = list(set(urls))
#for item in urls: 
#    print(item)
print('urls:' + str(len(urls)))
filename = '财政与税务学院.txt'
