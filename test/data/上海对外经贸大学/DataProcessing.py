...
def final_cleaning(school, fpath):
    '''输入学院的所有记录，进行清洗，并提取相应字段'''
    with open(fpath,'r',encoding='utf-8') as f:
        datas = f.readlines()
    datas = clean_null_list(clean_n(datas)) #初步清洗
#    new_datas = pd.DataFrame(columns=['性别'])
    new_datas = pd.DataFrame(columns=['姓名','性别','出生年月','学历','学位','毕业学校','所属学院','职称','职务','电子邮件','电话','工作单位（院校）','所有信息'])
    for data in datas:
        ...
        employer = school #学校
        all_message = data
        row = {'姓名':name,'性别':gender,'出生年月':birth,'学历':xueli,'学位':xuewei,'教育经历':Graduated_school,'所属学院':college,'职称':zhicheng,'职务':zhiwu,'电子邮件':email,'电话':phone,'工作单位（院校）':employer,'所有信息':all_message}
        new_datas = new_datas.append(row,ignore_index=True)
    new_datas.to_excel('clean/' + fpath[:-4] + '.xlsx')
...
final_cleaning('上海对外经贸大学','上海对外经贸大学.txt')
