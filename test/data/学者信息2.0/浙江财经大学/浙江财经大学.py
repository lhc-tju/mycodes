...
#            soup = BeautifulSoup(data,'lxml')
        ...
        #针对性更改,可设置多个result
        try:
            title = soup.select('.main-public-con-right-box-neirong-bt')[0]
...
#        soup = BeautifulSoup(res.text,'lxml')   
            ...
            data = drive.page_source
            soup = BeautifulSoup(data,'lxml')
            result = soup.select('#vsb_content_4')[0].find_all('a')
...
urls = ['https://rwxy.zufe.edu.cn/xygk/szdw.htm']
urls = get_urls(urls,'aa')
urls = list(set(urls))
#for item in urls: 
#    print(item)
print('urls:' + str(len(urls)))
filename = '人文与传播学院.txt'
