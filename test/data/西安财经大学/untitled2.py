...
#        soup = BeautifulSoup(res.text,'lxml') 
        ...
        try:
#            title = soup.find('h1')
            result = soup.find_all('tbody')[5].find_all('td')
...
#        soup = BeautifulSoup(res.text,'lxml')   
            ...
            data = drive.page_source
            soup = BeautifulSoup(data,'lxml')
#            result = soup.select('.list_right')[0].find_all('a')
...
urls = ['http://wenxue.xaufe.edu.cn/szfz.asp']
urls = get_urls(urls,'http://wenxue.xaufe.edu.cn/') 
urls = list(set(urls))
#for item in urls:
#    print(item)
