...
#            soup = BeautifulSoup(data,'lxml')
        ...
        #针对性更改,可设置多个result
        try:
#            title = soup.select('.arttitle')[0]
            result = soup.select('.col_news-s')[0].find_all('tr')
            r2 = soup.select('.col_news-s')[0].find_all('p')
#            r2 = soup.find('tbody').find_all('p')
        except:
            try:
                result = soup.select('.artmainl')[0].find_all('p')
            except:
...
#        soup = BeautifulSoup(res.text,'lxml')   
            ...
            data = drive.page_source
            soup = BeautifulSoup(data,'lxml')
            result = soup.select('.tab-list')[0].select('.cbox')[:2]
...
urls = ['http://spea.shufe.edu.cn/1130/list.htm']
urls = get_urls(urls,'http://spea.shufe.edu.cn')
urls = list(set(urls))
#for item in urls: 
#    print(item)
print('urls:' + str(len(urls)))
filename = '公共经济与管理学院-财政-税务系.txt'
